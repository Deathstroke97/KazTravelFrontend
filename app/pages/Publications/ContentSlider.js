        ...
        <Media minWidth={760}>
          {
            slidesAmount <= 4 ? (
              <div className="thumb-slider">
                {slides.map((el, i) => (
                  <div className={classNames('slick-slide', { 'slick-current': this.state.active === i })} key={id()}>
              ...
              <SlickSlider {...thumbSettings} ref={s => (this.slider2 = s)}>
                {slides.map((el, i) => (
                  <figure className="content-slider-thumb" key={id()} onClick={() => this.thumbClickHandler(i)}>
                    <img src={el.thumb} alt=""/>
                  </figure>
                ))}
              </SlickSlider>
